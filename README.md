...
Full wifi\_down-style launch banner plays once on startup, then drops you into the interactive recon-toolkit menu.
...
The SHIV banner system is modelled directly on [wifi\_down](https://github.com/amibhai/wifi_down).

### Launch sequence (runs once at startup)

```
1.  cls / clear screen
2.  SHIV block-art — revealed row-by-row with 0.04 s scan-line delay
...
```
  SHIV  ◈  recon-toolkit  ◈  2026-06-12  00:04:11  ◈  <target>
```
...
print_banner()               # full SHIV launch sequence (call once)
...
```
  SHIV  ◈  recon-toolkit  ◈  2026-06-12  00:15:32
...
Toggle inline PCAP capture from the **P** menu. When enabled, every recon-toolkit scan session writes a timestamped `.pcap` file to the configured directory:
...
Shows current user, effective UID, Scapy availability, and which recon-toolkit scan types are accessible without root.
...
Packet captures written during any recon-toolkit scan session:
...
├── CHANGELOG.md
│
├── menu.py                     # Interactive menu (wifi_down-style SHIV launch)
│
├── tools/
│   └── recon_menu.py           # Legacy entry point (kept for compatibility)
│
├── recon/
│   ├── core/
│   │   ├── logger.py           # SHIV banner system + Rich logger + JSON Lines
